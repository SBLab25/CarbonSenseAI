...
export const GoalProgressBar: React.FC<GoalProgressBarProps> = ({
  ...
  return (
    <div className="flex flex-col w-full bg-slate-900/60 rounded-2xl border border-slate-800 p-5 shadow-lg">
      <div className="flex justify-between items-center mb-3">
        <span className="text-sm font-semibold text-slate-400">Monthly Reduction Goal</span>
        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
          isAchieved ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'
        }`}>
      ...
      <div className="flex justify-between items-center text-xs text-slate-400 mt-1">
        ...
        <span className="font-semibold text-white">Target: {target.toFixed(1)}%</span>
      </div>
      
      {current < 0 && (
        <div className="mt-3 text-xs text-red-400 bg-red-950/20 border border-red-900/30 px-3 py-2 rounded-xl">
